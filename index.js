  ...
  function updateCountdown() {
    ...
    if (diff <= 0) {
      ...
      document.querySelectorAll(".countdown-marquee span").forEach(span => span.textContent = message);
      return;
    }
    ...
    const countdownMessage = `Next Release: ${days} days ${hours.toString().padStart(2, '0')} hours ` +
      `${minutes.toString().padStart(2, '0')} minutes ${seconds.toString().padStart(2, '0')} seconds`;

    document.querySelectorAll(".countdown-marquee span").forEach(span => span.textContent = countdownMessage);
  }
