// Junya Watanabe Scene Updater with Networking Support
console.log('[CLIENT] Loading junya-scene-updater.js...')
...
// Function to display avatar position info for debugging
function updateAvatarDebugInfo() {
  const avatars = document.querySelectorAll('[id^="avatar-"]')

  avatars.forEach((avatar) => {
    const position = avatar.getAttribute('position')
    const worldName = avatar.getAttribute('gltf-avatar')?.worldName

    if (position && worldName) {
      console.log(
        `Avatar in ${worldName}: Position ${JSON.stringify(position)}`
      )
    }
  })
}
...
window.onload = function () {
  // Run original scene update functionality
  setInterval(updateScene, 200)

  // Add optional debugging for avatars
  if (window.location.search.includes('debug=true')) {
    setInterval(updateAvatarDebugInfo, 2000)
  }
}
